...
class NetworkManager extends EventEmitter {
  ...
  constructor(client) {
    ...
    this._userResponseInterceptionEnabled = false;
    ...
    this._client.on('Network.requestIntercepted', this._onIntercepted.bind(this));
   ...
   * @param {boolean} value
   */
  async setResponseInterception(value) {
    this._userResponseInterceptionEnabled = value;
    await this._updateProtocolRequestInterception();
  }
  
  async _updateProtocolRequestInterception() {
    const enabled = this._userRequestInterceptionEnabled || this._userResponseInterceptionEnabled || !!this._credentials;
    if (enabled === this._protocolRequestInterceptionEnabled)
      return;
    this._protocolRequestInterceptionEnabled = enabled;
    const patterns = enabled ? [{urlPattern: '*', interceptionStage: 'Request'}, {urlPattern: '*', interceptionStage: 'HeadersReceived'}] : [];   
    await Promise.all([
   ...
   */
  _onIntercepted(event) {
    // Intercepted by headerReceived stage
    if (event.responseHeaders) {
      this._onBeforeResponse(event);
    }
    else{
      this._onRequestIntercepted(event);
    }
    return;
  }
   ...
   * @param {!Protocol.Network.requestWillBeSentPayload} event
   */
  _onBeforeResponse(event) {
    if(event.authChallenge){
      this._client.send('Network.continueInterceptedRequest', { 'interceptionId': event.interceptionId }).catch(debugError);
      return;
    }
    if (!this._userResponseInterceptionEnabled && this._protocolRequestInterceptionEnabled) {
      this._client.send('Network.continueInterceptedRequest', { 'interceptionId': event.interceptionId }).catch(debugError);
      return;
    }
    // if (event.resourceType.toLowerCase() === 'image' || event.resourceType.toLowerCase() == "stylesheet" || event.resourceType.toLowerCase() == "document" || event.resourceType.toLowerCase() == "media" ){
    //   this._client.send('Network.continueInterceptedRequest', { 'interceptionId': event.interceptionId });
    //   return;
    // }
    this._client.send('Network.getResponseBodyForInterception', { 'interceptionId': event.interceptionId })
      .then(originResponse => {
        let responseBody = originResponse.base64Encoded ? Buffer.from(originResponse.body, 'base64') : Buffer.from(originResponse.body, 'utf8');
        let beforeResponse = new BeforeResponse(this._client, event, responseBody.toString('utf8'));
        this.emit(Events.NetworkManager.BeforeResponse, beforeResponse);
      }).catch((err)=>{this._client.send('Network.continueInterceptedRequest', { 'interceptionId': event.interceptionId })}) // if failed continue directly e.g. HSTS rsp
  }
...
class BeforeResponse {
   ...
   */
  constructor(client, event, responseBody) {
    this._client = client;
    this._event = event;
    this.request = event.request;

    this.interceptionId = event.interceptionId;
    this.status = event.responseStatusCode || 200;
    this.statusText = statusTexts[event.responseStatusCode] || '';
    this.url = event.request.url;
    this.headers = event.responseHeaders;
  ...
  async continue(){
    let statusLine = `HTTP/1.1 ${this.statusCode} ${this.statusText}`;

    let responseBody = Buffer.from(this.body, 'utf8')

    this.headers['content-length'] = Buffer.byteLength(responseBody);

    const CRLF = '\r\n';
    let text = statusLine + CRLF;
    for (const header of Object.keys(this.headers))
      text += header + ': ' + this.headers[header] + CRLF;
    text += CRLF;
